...
In this case the tool is a [Shiny app](https://github.com/jamesphare/running_predictor/blob/master/running_predictor/app.R). For now, this app is very simple. The user simply enters the length of the race in kilometers and the number of kilometers I've trained in the preceding twelve weeks, and the app returns a predicted race time in kilometers per second.

The next steps are to polish this app, so that it returns minutes and seconds and allows toggling between kilometers and miles. It should also communicate uncertainty by adding (and perhaps visualizing) confidence intervals.

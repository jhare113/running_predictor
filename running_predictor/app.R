...
ui <- fluidPage(# Application title
    ...
    sidebarLayout(
        ...
        mainPanel(tags$h3(textOutput("pace")),
                  tags$h3(textOutput("finish")))
    ))
...
server <- function(input, output) {
    output$pace <- renderText({
        if (input$units == "kms") {
            ...
            race_pace <-
                342.06054 + (2.808069 * as.numeric(input$distance)) -
                (0.17157 * as.numeric(input$training))
            
            race_pace <- round(race_pace, 0)
            
            paste("Predicted race pace: ",
                  seconds_to_period(race_pace),
                  " / km",
                  sep = "")
        }
        else{
    ...
    output$finish <- renderText({
        if (input$units == "kms") {
            ...
            finish_time <-
                round(race_pace * as.numeric(input$distance), 0)
            
            paste("Predicted finish time: ",
                  seconds_to_period(finish_time),
                  sep = "")
